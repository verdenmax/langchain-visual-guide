...
LESSON_37_LOCAL_DEV = _build_lesson(
    {
        "lead": "读源码和贡献不是从克隆仓库后盲目全量测试开始，而是从一个公开 API 的归属开始：先判断它属于 langchain、<span class='mono'>langchain-core</span>、langgraph 还是本教程生成器，再进入对应包的 pyproject、测试目录和 CI 规则。工程化学习的目标不是记住每个目录，而是建立一条可复查路线：选择 API、找到包边界、用 editable 环境让本地源码参与 import、写或跑一个聚焦测试、用 CI 同款检查证明生成物没有漂移，最后把变更压成小 PR。本课把本地开发、源码调试、包布局和贡献循环合成一条新手可执行的工作流。",
        ...
        "source_intro": "下面的 source map 特意使用当前已验证路径。LangChain 主仓当前没有根目录 pyproject；包级配置在 <span class='mono'>libs/langchain_v1/pyproject.toml</span> 与 <span class='mono'>libs/core/pyproject.toml</span>，LangGraph 在独立 <span class='mono'>langchain-ai/langgraph</span> 仓库的 <span class='mono'>libs/langgraph/pyproject.toml</span>。本教程自己的工程守卫则在 <span class='mono'>src/build.py</span>、<span class='mono'>src/check_*</span> 和 <span class='mono'>.github/workflows/ci.yml</span>。",
        "sources": [
...
""",
        ...
        "sections": [
            ("先确认你在改哪个包", [
                "大型 Python monorepo 里最容易犯的错，是把 import 名称、发布包名称和仓库目录混在一起。<span class='mono'>langchain.agents.create_agent</span> 是用户 API，但它不是 <span class='mono'>langchain-core</span> 的稳定协议；<span class='mono'>Runnable</span>、<span class='mono'>AIMessage</span>、<span class='mono'>BaseCallbackHandler</span> 属于 <span class='mono'>langchain-core</span>；checkpoint 和图执行能力来自 LangGraph。贡献前先写下三列：用户 import、源码文件、发布包。三列对齐，后面的测试和 reviewer 说明才不会跑偏。",
                "包边界还决定兼容性承诺。core 的符号被很多包依赖，修改参数、返回类型或异常语义会影响广泛；高层 <span class='mono'>langchain_v1</span> 可以组合 core 和 langgraph，但也要保护用户 API；本教程 src 目录则是生成器，修改内容后要提交生成的 HTML。不同边界对应不同风险，不应同样处理。",
            ]),
            ("Editable install 的意义不是省安装时间", [
                "editable install 的关键价值是让解释器导入你的工作区源码。没有这一步，你可能在编辑 A 文件，却一直运行环境里旧 wheel 的代码，断点打不到、print 不出现、测试也不会反映修改。确认方式很简单：在 Python 中打印目标模块的 <span class='mono'>__file__</span>，路径必须指向当前仓库或克隆目录，而不是 <span class='mono'>site-packages</span> 里的压缩发布物。",
                "LangChain 与 LangGraph 的 pyproject 中都有 <span class='mono'>tool.uv.sources</span> 指向相邻包的本地路径。这个配置说明维护者希望开发时让多个包以源码方式联动：改 core 后 <span class='mono'>langchain_v1</span> 立即看到，改 prebuilt 或 checkpoint 后 langgraph 测试能覆盖。若这些来源路径和你本地目录不一致，要先修环境，不要急着怀疑业务代码。",
            ]),
            ("聚焦测试比全量测试更适合调试", [
                ...
                "本教程的聚焦测试是 <span class='mono'>build.py</span>、<span class='mono'>build_print.py</span>、<span class='mono'>check_links.py</span>、<span class='mono'>check_html.py</span>、<span class='mono'>check_content_density.py</span>。内容类改动最常见失败不是 Python 逻辑，而是生成 HTML 没提交、链接断了、课程缺少 source_map 或 CJK 密度不足。先跑这些检查，能在本地发现 CI 会拒绝的 drift。",
            ]),
            ("源码调试从入口往内钻", [
                ...
                "断点也要按状态边界放。对 Agent 问题，先看 <span class='mono'>create_agent</span> 收到的 tools、middleware、<span class='mono'>context_schema</span>；再看模型节点输出的 <span class='mono'>AIMessage</span>；再看 <span class='mono'>ToolMessage</span> 是否写回；最后看 <span class='mono'>structured_response</span>。对静态教程问题，先看 <span class='mono'>registry.CONTENT</span> 是否映射正确，再看 build 输出，再看检查器解析到的原始 HTML。",
            ]),
            ...
            ("本教程的生成物也是源代码责任", [
                "很多静态站点把 HTML 当产物不提交，本仓库相反：<span class='mono'>index.html</span> 和 <span class='mono'>lessons/</span> 是需要随 src 同步提交的发布内容，CI 会重新运行 build，并用 <span class='mono'>git diff</span> 检查这两者是否过期（<span class='mono'>print.html</span> 在 CI/deploy 都会用 <span class='mono'>build_print.py</span> 重建，但不纳入漂移检查，部署时用于生成 PDF）。这意味着改 <span class='mono'>src/part08_engineering.py</span> 或 <span class='mono'>src/quizzes.py</span> 后，未重新生成 <span class='mono'>index.html</span>/lessons 就提交，属于不完整变更。",
                "生成物同步不是形式主义。读者直接打开 <span class='mono'>index.html</span> 或 GitHub Pages 时看到的是 HTML，不是 Python 源。若源码和 HTML 漂移，reviewer 看源觉得正确，用户却读到旧页面。CI 的 diff gate 把这种风险提前暴露，等价于软件项目里检查编译产物或 API schema 是否随源更新。",
            ]),
            ("常见误解与边界情况：环境问题不是业务问题", [
                "常见误解是“测试没失败，所以源码路径肯定对”。如果你没有确认 <span class='mono'>module.__file__</span>，测试可能跑的是已安装版本；如果你没有让最小复现先红，测试可能没有覆盖目标行为。另一个误解是“全量 CI 失败一定是我这行代码错了”。在大型仓库里，环境、缓存、依赖约束、生成物漂移都可能导致失败，必须先用聚焦检查缩小范围。",
            ...
            ("从本地失败到教学素材", [
                "工程化学习的一个技巧，是把每次失败都转成 source map 的一行。今天因为 check_content_density 失败，就记住它从 <span class='mono'>registry.CONTENT</span> 读原始 lesson body 而不是读 <span class='mono'>lessons/</span>*.html；明天因为链接失败，就记住 <span class='mono'>check_links.py</span> 解析所有相对 href。失败越具体，你的源码地图越可靠。",
                "同样方法也适用于 LangChain。一次 no-tool Fake 模型测试失败，可以补充 <span class='mono'>GenericFakeChatModel</span> 的行为；一次 Agent 工具循环失败，可以补充 <span class='mono'>FakeToolCallingModel</span>、<span class='mono'>AIMessage.tool_calls</span> 和 <span class='mono'>ToolMessage</span> 的配对规则。读源码不是一次性任务，而是把调试证据持续沉淀成可复查路径。",
            ]),

            ("多包仓库里的调试礼仪", [
                "在多包仓库中工作，还要尊重依赖方向。不要为了让一个高层测试通过，就让 <span class='mono'>langchain-core</span> 反向导入 <span class='mono'>langchain_v1</span> 或某个 partner；不要为了省事在 core 里读取环境变量、调用具体 provider SDK 或依赖图运行时。稳定层越干净，边缘包越能独立演进。调试时若发现自己想从底层 import 高层，通常说明修复点选错了，应该把适配逻辑留在上层，或通过协议把需要的信息向下传递。",
                ...
                "source debugging 也包括理解失败来自构建系统还是运行系统。本教程的失败可能来自 HTML 结构、导航链、密度门槛；LangChain 的失败可能来自 <span class='mono'>mypy</span>、<span class='mono'>ruff</span>、<span class='mono'>pytest</span>、snapshot、依赖约束。先给失败分类，再决定修代码、修测试、修生成物还是修环境。没有分类的调试会在同一堆日志里反复横跳。",
            ]),
            ("贡献记录应留下学习路径", [
                "一份好的贡献记录能让后来者复用你的路线。假设你修了一个 tool schema 问题，PR 描述可以写：公开入口是 <span class='mono'>@tool</span>，契约在 <span class='mono'>StructuredTool</span>，失败测试在某个单测，修复影响参数描述生成，不改变工具调用返回类型。这样的记录不只是给 reviewer 看，也会在半年后帮助另一个新手快速进入上下文。",
            ...
            ("把源码阅读变成可复制路线", [
                ...
                "路线还要记录没有走的路。例如本次核验发现 LangChain 主仓没有根 pyproject，计划中旧路径就必须标记为过期；发现 LangGraph 在独立仓库，就不能继续写 <span class='mono'>langchain/libs/langgraph</span>。把这些负面发现写下来，可以防止后来的读者重复踩坑。工程化学习不只是知道正确答案，也包括知道哪些旧答案已经失效。",
        ...
        "points": [
            ...
            "editable install 要用 <span class='mono'>module.__file__</span> 验证，不能凭感觉。",
...
LESSON_38_TESTING_DEBUGGING = _build_lesson(
    {
        "lead": "测试 Agent 不是把真实模型、真实网络和真实数据库都接上后祈祷输出稳定，而是把不确定性逐层替换成可控组件。这里要先分清两类 fake：<span class='mono'>GenericFakeChatModel</span> 适合不带工具绑定的模型测试、token/callback 测试和消息脚本测试；Agent-with-tools 循环会调用模型的 <span class='mono'>bind_tools</span>，因此需要 <span class='mono'>FakeToolCallingModel</span> 这类实现 <span class='mono'>bind_tools</span> 的测试替身，或项目内自定义一个明确标注为 test fake 的 <span class='mono'>ToolCallingFakeModel</span>。确定性工具返回固定观察，<span class='mono'>InMemorySaver</span> 保存线程状态，<span class='mono'>Runnable</span> 接口让每个节点可单独 <span class='mono'>invoke</span>，<span class='mono'>create_agent</span> 再把这些零件装成完整循环。这样写出的测试关注结构行为：模型是否请求了正确工具、工具观察是否写回 <span class='mono'>messages</span>、checkpoint 是否能恢复、回归案例是否锁住曾经失败的 trace，而不是脆弱地断言某一句自然语言完全相同。",
        ...
        "source_intro": "测试课的 source map 选择运行契约和测试替身。<span class='mono'>GenericFakeChatModel</span> 在 <span class='mono'>langchain-core</span>，适合无工具绑定的固定消息、token 与 callback 测试；LangChain v1 的单测还提供 <span class='mono'>tests.unit_tests.agents.model.FakeToolCallingModel</span>，专门给 <span class='mono'>create_agent</span> + tools 场景使用，因为它实现 <span class='mono'>bind_tools</span>。<span class='mono'>AIMessage</span>、<span class='mono'>Runnable</span>、<span class='mono'>create_agent</span> 和 <span class='mono'>InMemorySaver</span> 则帮助你把失败定位到消息结构、可运行接口、Agent 工厂或图状态。",
        "sources": [
...
""",
        ...
        "sections": [
            ("Fake 模型测试的是框架行为，不是模型智商", [
                ...
                "选择 fake 时要看测试入口是否会绑定工具。<span class='mono'>GenericFakeChatModel</span> 是轻量的 <span class='mono'>BaseChatModel</span> fake，适合“给定消息脚本，验证 token callback、stream 或无工具链路”的场景；<span class='mono'>create_agent</span>(model, tools=[...]) 会先把工具 schema 绑定到模型，因此 fake 必须实现 <span class='mono'>bind_tools</span>。上游 <span class='mono'>FakeToolCallingModel</span> 就是为 Agent 工具循环准备的测试模型。",
            ...
            ("Trace 断言优先于最终文本断言", [
                "最终文本是最不稳定的断言对象。即使用 Fake 模型，真实项目中你也可能换 prompt 或换 provider，导致同义句变化。更稳的断言是结构：第一轮 <span class='mono'>AIMessage</span> 是否包含正确 <span class='mono'>tool_calls</span>，<span class='mono'>ToolMessage.tool_call_id</span> 是否配对，消息数量是否按预期增长，最终 <span class='mono'>AIMessage</span> 是否不再请求工具，<span class='mono'>structured_response</span> 是否含必需字段。",
                "结构断言还能定位失败层。如果 <span class='mono'>tool_calls</span> 参数错，问题在模型脚本或 prompt；如果 <span class='mono'>ToolMessage</span> 缺失，问题在工具节点或路由；如果最终响应字段缺失，问题在 <span class='mono'>response_format</span> 或最终模型消息。只断言“回答包含已发货”无法提供这种定位能力。",
            ]),
            ("<span class='mono'>InMemorySaver</span> 适合测试，不适合伪装生产持久化", [
                "<span class='mono'>InMemorySaver</span> 的优势是轻、快、无需外部服务，适合测试一个 <span class='mono'>thread_id</span> 下的状态保存和恢复。它能帮助你断言第二次调用是否看到第一次 <span class='mono'>messages</span>，或者 <span class='mono'>update_state</span> 后图是否从正确快照继续。因为它在进程内存中，测试结束就消失，正好避免污染环境。",
                "但它不代表生产持久化能力。跨进程、重启恢复、并发写入、权限隔离和长期审计需要数据库或专门 checkpoint 后端。测试中使用 <span class='mono'>InMemorySaver</span> 时，要在命名上明确这是 test saver，避免新人误以为把它放进服务就完成了记忆系统。",
            ]),
            ("回归案例来自真实失败，但要压缩", [
                "线上失败往往包含大量无关历史、隐私字段和真实工具结果。写回归测试时，不应直接复制整段日志，而要压缩成最小消息序列：保留触发失败所需的 <span class='mono'>HumanMessage</span>、<span class='mono'>AIMessage.tool_calls</span>、<span class='mono'>ToolMessage</span> 或 context 字段，删除无关内容并脱敏。最小案例更稳定，也更容易让未来维护者理解。",
                "压缩后要确认测试先失败。若测试一写就通过，说明它没有覆盖真实 bug，或者现有代码已经通过其他路径修复。TDD 的红灯在回归场景尤其重要：它证明你的测试确实能抓住曾经的缺陷，而不是只给 coverage 数字添砖。",
            ]),
            ("调试顺序：从可单独 <span class='mono'>invoke</span> 的边界开始", [
                "<span class='mono'>Runnable</span> 的统一接口让你不必每次都跑完整 Agent。先单独 <span class='mono'>invoke</span> prompt，确认消息渲染；单独调用 Fake 模型，确认脚本顺序；单独调用工具，确认 fixture；单独检查 <span class='mono'>checkpointer</span>，确认 <span class='mono'>thread_id</span>；最后再运行 <span class='mono'>create_agent</span> 组装结果。分层调试比在端到端测试里加大量 print 更清楚。",
                "当端到端测试失败时，按最近新增状态逆向排查。最终回答错，先看最后一条 <span class='mono'>AIMessage</span>；工具观察没被用，先看 <span class='mono'>ToolMessage</span> 内容是否可读；工具没被调用，先看 Fake 模型第一条消息是否真的有 <span class='mono'>tool_calls</span>；状态丢失，先看 <span class='mono'>config.configurable.thread_id</span> 是否一致。",
            ]),
            ("常见误解与边界情况：Mock 不是越多越好", [
                "常见误解是“所有依赖都 mock 掉，测试就稳定”。过度 mock 会让测试只验证 mock 的调用姿势，而不是真实契约。<span class='mono'>GenericFakeChatModel</span> 是有价值的 no-tool fake，因为它实现聊天模型接口并返回真实 <span class='mono'>AIMessage</span>；但工具循环要换成实现 <span class='mono'>bind_tools</span> 的 <span class='mono'>FakeToolCallingModel</span>/<span class='mono'>ToolCallingFakeModel</span>。内存工具 fixture 也有价值，因为它仍通过 <span class='mono'>@tool</span> schema。相反，直接 mock 掉 <span class='mono'>create_agent</span> 内部方法，往往会绕开你真正想验证的 Agent 循环。",
                "边界情况是 provider-specific 行为。不同模型对 <span class='mono'>tool_calls</span>、stream chunk 或结构化输出支持不同，单元测试可以用 fake 锁住框架预期，但仍需要少量 provider 集成测试覆盖转换层。测试金字塔不是只要底层，而是让大量稳定单测支撑少量昂贵集成测试。",
            ]),
            ("消息状态断言的粒度", [
                "一个实用断言模板包括：<span class='mono'>messages</span> 第一条是否为用户问题；第一条 <span class='mono'>AIMessage</span> 是否包含预期工具名和参数；工具结果是否用同一个 <span class='mono'>tool_call_id</span> 回写；最终 <span class='mono'>AIMessage</span> 是否不再包含 <span class='mono'>tool_calls</span>；如果有 <span class='mono'>structured_response</span>，字段是否满足 schema。这个模板比断言字符串完整相等更能表达 Agent 的真实契约。",
                "还要断负例。未知订单应返回澄清或拒绝，而不是继续调用退款工具；工具抛错应被记录为可读错误或触发重试，而不是吞掉；guard middleware 拦截后不应执行副作用工具。负例证明边界存在，正例只证明 happy path 能跑。",
            ]),
            ("把测试输出写成可读证据", [
                "失败信息要帮助定位。断言 <span class='mono'>expected_tool_names</span> == <span class='mono'>actual_tool_names</span>，比 assert len(<span class='mono'>tool_calls</span>) == 1 更有用；断言 message types 序列等于 ['human','ai','tool','ai']，比只看最终内容更直观。好的测试失败时像小 trace，不需要开发者重新跑 debugger 才知道状态断在哪里。",
                "测试名也要描述行为，例如 <span class='mono'>test_agent_uses_order_tool_before_final_answer</span>，而不是 <span class='mono'>test_agent</span>。行为名会在 CI 输出里变成导航。未来有人改 middleware 或工具 schema 导致失败时，测试名直接告诉他破坏的是哪条工程契约。",
            ]),

            ("从失败输出反推测试设计", [
                "设计 Agent 测试时，可以先写理想失败输出。比如你希望失败时看到“期望工具 <span class='mono'>search_order</span> 被调用一次，实际调用了 <span class='mono'>search_order</span> 两次，第二次参数 <span class='mono'>order_id</span> 为空”。为了得到这样的输出，测试就不能只断言最终 answer，而要收集工具调用记录、参数摘要和消息序列。好的测试不是事后装饰，它会反过来推动代码暴露清晰边界。",
                "当测试难写到必须 mock 一堆私有方法时，往往说明设计边界不清。也许工具把查询、判断和副作用混在一起；也许 prompt 渲染和模型调用无法分开；也许 context 安全字段被塞进普通消息。不要急着堆 mock，而要把组件拆到可以单独 <span class='mono'>invoke</span> 的程度。测试困难是设计反馈，不是测试作者的问题。",
                "回归测试还要控制断言粒度。太粗会漏 bug，太细会因无关重构失败。经验是锁住协议字段而非实现细节：消息类型、工具名、参数、id、结构化字段、错误类别属于协议；内部局部变量名、具体日志句子、自然语言标点通常不是协议。",
            ]),
            ("把测试金字塔放进 Agent 语境", [
                "Agent 项目的底层应有大量纯函数和 <span class='mono'>Runnable</span> 单测：prompt 渲染、工具参数校验、retriever 格式化、middleware guard、response schema。中层是 fake Agent 流程测试，验证 model -> tools -> model、checkpoint、结构化输出。顶层才是少量真实 provider 集成测试和人工评测，用来发现 fake 无法覆盖的模型行为。",
            ...
            ("调试 Agent 时的最小观察面", [
                "最小观察面不是所有日志，而是足以重建控制流的字段。对 Agent 来说，至少要看到消息类型序列、最新 <span class='mono'>AIMessage</span> 的 <span class='mono'>tool_calls</span>、每个 <span class='mono'>ToolMessage</span> 的 <span class='mono'>tool_call_id</span>、工具参数摘要、checkpoint <span class='mono'>thread_id</span>、最终结构字段和错误类别。有了这些字段，即使不看完整自然语言，也能判断循环为何继续、为何停止、状态是否保存。",
                "测试代码可以把这些字段整理成小型 trace，再做快照或显式断言。比如 <span class='mono'>expected_trace</span> = [('human', None), ('ai', '<span class='mono'>search_order</span>'), ('tool', '<span class='mono'>call_1</span>'), ('ai', None)]。这种断言一眼就能看出多了一次工具调用或缺了一条观察，比翻完整消息对象更适合回归。",
                "当失败与流式输出有关时，还要区分 chunk 与最终消息。<span class='mono'>GenericFakeChatModel</span> 能用于 token callback 这类无工具绑定测试，但你的业务断言不应把中间 chunk 当最终状态。测试应等待最终聚合结果，再断言 <span class='mono'>structured_response</span>；如果同一用例还要走工具循环，就改用带 <span class='mono'>bind_tools</span> 的 test fake，并把 callback 断言与工具断言分开。",
            ]),

            ("测试命名、fixture 与可维护性", [
                "测试长期可维护，靠的不只是断言，还靠命名和 fixture 组织。fixture 名称应说明业务含义，例如 <span class='mono'>delayed_order</span>、<span class='mono'>eligible_refund</span>、<span class='mono'>blocked_refund</span>，而不是 <span class='mono'>data1</span>、<span class='mono'>data2</span>。Fake 模型脚本也应按阶段命名，例如 <span class='mono'>ai_requests_order_tool</span>、<span class='mono'>ai_final_answer</span>。这样测试失败时，读者不用先理解一大段 JSON，单看名字就知道场景。",
                "fixture 还要避免共享可变状态污染。Fake 模型迭代器、<span class='mono'>InMemorySaver</span>、工具调用记录最好每个测试重新创建；若多个测试共用同一个对象，前一个测试消耗了一条消息，后一个测试就会莫名失败。确定性测试不仅要求外部世界稳定，也要求测试自身状态隔离。",
            ...
            ("并发、重试和恢复边界", [
                "测试还应覆盖并发和重复调用的边界。即使当前示例只有一个工具，也要理解 <span class='mono'>tool_call_id</span> 为什么存在，为什么工具观察必须和请求配对，为什么 checkpoint 要按 <span class='mono'>thread_id</span> 隔离。很多线上事故不是 happy path 不会跑，而是重试、并发、恢复和错误分支没有被测试。",
                "工具调用 fake 也要覆盖这些边界：可以脚本化两次相同 <span class='mono'>tool_call_id</span> 来证明系统会拒绝或覆盖异常；可以让第一轮工具返回错误观察、第二轮模型改参重试；可以使用两个 <span class='mono'>thread_id</span> 运行同一 fake 工厂，确认 <span class='mono'>checkpointer</span> 不会把消息串到另一个用户。这样的测试才真正保护 Agent 循环，而不是只保护一次 happy path。",
            ]),

            ("工具契约和 fake 脚本的同步", [
                "继续扩展测试时，要为每个工具维护一份小契约：输入字段、可信字段来源、正常返回、可重试错误、不可重试错误、副作用语义。Fake 测试按契约构造消息，集成测试再验证真实工具是否遵守契约。这样模型脚本、工具实现和回归断言不会各说各话。",
                "当工具 schema 改名、参数从字符串变成对象，或权限字段迁到 runtime context 时，fake 脚本必须同步更新；否则测试会继续验证旧协议。维护者可以把工具契约放进 fixture builder：同一处生成 <span class='mono'>@tool</span>、合法参数、非法参数和期望 <span class='mono'>ToolMessage</span>。这样测试数据与工具定义一起演进，减少“测试绿但真实 Agent 绑定失败”的风险。",
            ]),
        ],
        "pitfalls": [
        ...
        "points": [
            "<span class='mono'>GenericFakeChatModel</span> 适合 no-tool 模型测试；Agent 工具循环需要实现 <span class='mono'>bind_tools</span> 的 fake。",
            "确定性工具应去掉网络、时间和副作用。",
            "Trace 断言比最终文本断言更稳定、更可定位。",
            "<span class='mono'>InMemorySaver</span> 适合测试状态恢复，不代表生产持久化。",
            "回归测试要从真实失败压缩，并先看到红灯。",
        ],
    }
)
...
LESSON_39_OBSERVABILITY_CI = _build_lesson(
    {
        "lead": "可观测性和 CI 是同一件事的两面：前者让一次运行的内部过程可解释，后者让一次代码变更的发布过程可验证。Callbacks 和 tracer 把 <span class='mono'>Runnable</span>、模型、工具、链、Agent 的 start/end/error 事件组织成 run tree；本教程的 build、check_links、check_html、check_content_density 和 CI diff gate 则把课程源码到生成 HTML/PDF 的过程组织成提交树。工程化不是等线上出错再翻日志，而是在每次运行和每次 PR 中留下足够证据，能回答“谁调用了谁、输入输出是什么、哪里漂移、为什么拒绝发布”。",
        ...
        "source_intro": "本课把上游可观测性接口和本仓库 CI 守卫放在同一张图里。<span class='mono'>BaseCallbackHandler</span> 与 <span class='mono'>BaseTracer</span> 来自 <span class='mono'>langchain-core</span>；check_html、check_links、check_content_density 和 <span class='mono'>ci.yml</span> 来自当前教程仓库，路径已经按当前版本验证。",
        "sources": [
...
""",
        ...
        "sections": [
            ("Callback 和 middleware 的边界", [
                "Callback 主要负责观察：某个 <span class='mono'>Runnable</span> 开始、模型收到哪些消息、工具何时结束、错误在哪里发生。它不应悄悄改写业务输入，也不应把安全策略藏在日志代码里。Middleware 主要负责改变或包裹运行：注入动态提示、拦截工具、限流、重试、替换模型、读取 runtime context。二者都在运行路径上，但职责不同。",
            ...
            ("Run tree 是嵌套调用的地图", [
                "复杂链路不是一条线，而是一棵树：外层 Agent run 下面有 model run、tool run、retriever run、parser run；工具内部也可能调用子 <span class='mono'>Runnable</span>。<span class='mono'>BaseTracer</span> 的价值是保存父子关系、<span class='mono'>run_id</span>、输入输出、开始结束和错误，让一次失败能从根节点一路展开到具体子节点。",
            ...
            ("观测数据要最小化和脱敏", [
                "可观测性不是把所有 prompt、用户输入、工具返回和密钥原样写进日志。工程上应采用白名单和摘要策略：记录消息类型、长度、工具名、参数键、文档 id、错误类别、耗时和 <span class='mono'>run_id</span>；对用户内容、系统提示、token、订单号等敏感字段做脱敏或不记录。",
                "这不是和排障矛盾。好的 trace 保留结构证据而不暴露秘密。例如记录 retriever 返回 <span class='mono'>doc_id</span> 和 score，必要时让授权工程师按 <span class='mono'>doc_id</span> 回查；记录工具参数里的 <span class='mono'>order_id</span> 哈希，而不是完整个人信息。可观测性越强，越需要治理边界。",
            ]),
            ("本地 CI 是提交前的保险", [
            ...
            ("Generated drift 是静态站点的真实 bug", [
                "源码生成型项目最典型的错误，是改了源数据却没有提交生成结果。用户打开的是旧 HTML，reviewer 看的是新 Python，二者不一致。CI 的 <span class='mono'>git diff</span> gate 明确把这种情况视为失败：重新 build 后若 <span class='mono'>index.html</span> 或 <span class='mono'>lessons/</span> 有差异，说明提交不完整。",
                "<span class='mono'>print.html</span> 和 PDF 也属于同一条链。<span class='mono'>build_print.py</span> 把课程合成可打印源，deploy 再用浏览器生成 PDF。若课程 HTML 更新但 print 没更新，在线页面和下载 PDF 会不一致。对教程类仓库来说，内容一致性就是产品质量。",
            ]),
            ("Density gate 不是凑字数", [
                ...
                "密度检查读取的是 <span class='mono'>registry.CONTENT</span> 里的原始 body，而不是生成后的 lessons HTML。这避免导航、页脚、quiz 等外壳文字污染课程正文统计。写作者因此必须把真正内容放进 lesson 常量，而不是靠页面模板填充。",
            ]),
            ("常见误解与边界情况：观测不是自动正确", [
                "常见误解是“接了 LangSmith 或 tracer，系统就可控了”。观测只能告诉你发生了什么，不能替你决定策略是否正确。一个 Agent 反复调用同一工具，trace 会清楚显示循环，但修复仍要回到 prompt、工具描述、middleware 或 <span class='mono'>recursion_limit</span>。没有行动策略的观测，只是更漂亮的事故录像。",
            ...
            ("把运行证据和提交证据对应起来", [
                ...
                "证据链越完整，沟通越少依赖记忆。PR 描述贴出本地检查命令，线上告警带 <span class='mono'>run_id</span> 和工具名，reviewer 或值班工程师就能接着查。没有证据链，团队只能在聊天记录里反复问“你刚才跑了什么”“用户到底输入了什么”。",
            ]),
            ("观测预算和信号质量", [
                ...
                "信号质量比信号数量更重要。记录“工具失败”不如记录工具名、错误类别、是否可重试、参数摘要和 <span class='mono'>run_id</span>；记录“密度失败”不如记录哪个页面、当前 CJK、需要 CJK、缺哪个组件。检查器和 tracer 的输出都应让人下一步知道该打开哪个文件或哪个 run，而不是只制造噪音。",
                "CI 也要避免假绿。若 check_html 只检查生成页面不检查导航链，重排课程可能留下断裂；若 density 统计整页包括导航和 quiz，空正文也可能过线。本仓库选择读取 raw <span class='mono'>CONTENT</span> 并检查语义 class，就是为了提高信号质量，让绿色真正代表课程正文达标。",
            ]),

            ("CI 输出也要面向作者体验", [
                "一个好的 gate 不只是失败，还要告诉作者怎样修。比如 generated drift gate 输出 <span class='mono'>git diff</span> --stat，让作者知道哪些 HTML 过期；density gate 输出页面名、当前 CJK 和缺失组件，让作者知道是补正文还是补 source_map；link check 输出具体页面和 href，让作者能直接打开文件。检查器越贴近修复动作，团队越愿意在本地运行。",
            ...
            ("从指标到行动闭环", [
                ...
                "CI 也应定期复盘门槛是否仍合理。页面迁移增加后，<span class='mono'>C_LEVEL_PAGES</span> 要扩展；课程数量变化后，<span class='mono'>README</span> 和 print 计数要同步；上游路径变化后，source rows 要更新。守卫不是一次写完就永远正确，它也需要随项目演进维护。把守卫当成代码，而不是当成外部仪式，质量体系才会持续有效。",
            ...
            ("发布后的反馈回流", [
                ...
                "反馈回流要有明确入口：线上 <span class='mono'>run_id</span> 可以变成回归 trace，常见 CI 失败可以变成检查器提示，读者报告的死链可以变成链接检查样例。没有入口的反馈会停在聊天记录里；有入口的反馈会变成下一次发布前自动执行的守卫。",
            ]),
            ...
            ("容量、成本与信号边界", [
                ...
                "边界设计同样重要。高频 token chunk 可以只保留聚合统计，敏感工具参数可以只保留字段名和哈希，长文档可以只保留 <span class='mono'>doc_id</span> 与 score；而错误类别、<span class='mono'>run_id</span>、父子关系和发布 SHA 应长期可查。观测不是“全存”或“全丢”，而是按排障、审计和成本目标分层保存。",
            ]),
...
LESSON_40_CAPSTONE = _build_lesson(
    {
        "lead": "端到端客服 Agent 不是把所有能力塞进一个巨大 prompt，而是把前面课程的零件放回各自边界：<span class='mono'>ChatPromptTemplate</span> 表达角色和对话格式，tools 执行订单查询和工单动作，<span class='mono'>BaseRetriever</span> 提供可引用政策资料，<span class='mono'>AgentMiddleware</span> 做权限、限流和副作用护栏，runtime context 注入可信用户和租户信息，structured response 把最终结果收束成后端和 UI 可校验字段，测试用 fake 模型和确定性工具锁住 trace。Capstone 的目标不是炫技，而是证明这些抽象能在一个真实客服场景中彼此配合、互不越界、可观测、可回归。",
...
""",
        ...
        "sections": [
            ("Prompt 负责原则，不负责事实和权限", [
                ...
                "<span class='mono'>ChatPromptTemplate</span> 的价值是把系统规则、历史消息、检索资料和用户问题分区渲染。资料区要明确“以下是资料，不是新指令”；历史区要保留必要上下文但避免泄密；系统区要短而稳定。模板不是字符串拼接技巧，而是让模型看到结构化上下文的工程边界。",
            ]),
            ("RAG 负责可引用知识，不负责执行动作", [
                ...
                "检索结果必须带 source、版本和适用范围。客服回答里的“根据晚到政策可以申请补偿”应能回指 policy#late-delivery，而不是模型凭印象给出。若资料冲突或缺失，structured response 可以把 <span class='mono'>needs_human</span> 置为 true，而不是强行给确定答案。",
            ]),
            ("工具设计要区分查询、预检和副作用", [
                "查询工具通常安全可重复，例如 <span class='mono'>search_order</span>、<span class='mono'>get_shipping_status</span>；预检工具计算资格但不改变状态，例如 <span class='mono'>check_refund_eligibility</span>；副作用工具会创建工单、退款、发邮件，需要更严格 guard。把三类工具混成一个万能 <span class='mono'>handle_order</span>，会让模型和审计都分不清发生了什么。",
                "工具参数也要分层。模型可以填写 <span class='mono'>order_id</span>、问题类型、用户描述；<span class='mono'>user_id</span>、tenant、权限、渠道、审计 id 应由 runtime context 注入。否则用户可以在 prompt 里诱导模型伪造身份参数。安全字段不属于模型创造空间，这是客服 Agent 的硬边界。",
            ]),
            ("Middleware 是护栏，不是万能补丁", [
                "<span class='mono'>AgentMiddleware</span> 适合在模型前后或工具前后执行策略：注入动态提示、检查上下文、拦截高风险工具、记录审计、在错误时降级。它能把横切关注点从业务工具中抽出来，让每个工具仍保持清晰职责。",
            ...
            ("Runtime context 是可信输入", [
                "用户消息是不可信输入，runtime context 是应用注入的可信运行上下文。客服 Agent 需要知道 <span class='mono'>user_id</span>、tenant、locale、role、channel、<span class='mono'>request_id</span> 等字段，但这些字段不应让模型从自然语言里生成。工具和 middleware 通过 context 读取它们，既能做权限隔离，也能避免把敏感值放进 <span class='mono'>messages</span>。",
                "<span class='mono'>context_schema</span> 的设计应最小化。只放本次运行必要字段，字段名清楚，类型明确，默认不写入 prompt。若模型确实需要看到某个非敏感偏好，例如语言或时区，可由 middleware 脱敏后注入动态提示。密钥、内部权限和完整身份信息永远留在运行时侧。",
            ]),
            ("结构化响应让 UI 和后端不用猜", [
                "客服最终结果不应只是自然语言。UI 需要知道是否转人工、后端需要知道是否创建工单、评测需要知道引用、风控需要知道风险原因。<span class='mono'>response_format</span> 把这些字段声明成 schema，让模型输出可校验对象。自然语言 answer 仍存在，但它只是字段之一。",
                "结构化不等于正确。schema 能保证有 <span class='mono'>citations</span> 字段，不能保证引用真的支持结论；能保证 <span class='mono'>needs_human</span> 是布尔值，不能保证策略判断正确。因此结构化响应后仍要做引用核验、业务规则校验和回归测试。它解决的是形状，不是事实全部。",
            ]),
            ("可观测性要覆盖组件边界", [
                "一次客服运行至少要记录：<span class='mono'>request_id</span>、user/tenant 摘要、检索 query 和 doc ids、工具名和参数摘要、guard 决策、<span class='mono'>structured_response</span> 字段、错误和耗时。这样用户投诉“Agent 擅自退款”时，团队能确认是否真的调用了副作用工具，是否被 middleware 放行，依据是哪条政策。",
            ...
            ("测试从 happy path 到防线", [
                ...
                "测试也要覆盖 message state。断言工具调用顺序、<span class='mono'>ToolMessage</span> id、引用字段、<span class='mono'>needs_human</span>、<span class='mono'>risk_reason</span>，而不是只断言 answer 包含“抱歉”。客服系统最危险的失败常发生在结构层：该拦截的副作用没拦、该转人工的没有转、引用为空却给了肯定答案。",
            ]),
            ("常见误解与边界情况：端到端不等于一坨代码", [
                "常见误解是 Capstone 要展示一个巨大的全能 Agent 文件。真正的端到端是边界完整：prompt、tools、retriever、middleware、context、response、tests 各自短小清晰，通过 <span class='mono'>create_agent</span> 装配。文件越大、职责越混，越不适合作为毕业项目。",
                "边界情况是工具和 RAG 都能回答同一问题。例如“我的包裹什么时候到”既可能查订单工具，也可能查物流政策。策略应写清：实时个人状态优先工具，通用规则优先 RAG，二者冲突时展示具体状态并引用政策。不要让模型临场决定事实优先级。",
            ]),
            ("从玩具到生产的扩展点", [
                "玩具版本可以用内存 fixture、Fake 模型和 <span class='mono'>InMemorySaver</span>；生产版本要替换为真实 provider、数据库工具、持久 <span class='mono'>checkpointer</span>、权限服务、LangSmith 或自建 trace、评测集和报警。因为抽象边界相同，替换可以逐步发生，而不是重写整个 Agent。",
                "最先撑不住的通常是准确性、成本和可控性。准确性靠 RAG 评测和引用核验；成本靠缓存、工具选择和模型分层；可控性靠 middleware、人审和结构化响应。Capstone 不是终点，而是让这些扩展点有地方插。",
            ]),

            ("组件契约如何支撑团队协作", [
                "客服 Agent 往往不是一个人维护。平台工程师维护 middleware 和 context，搜索团队维护 retriever，业务后端维护订单工具，产品或内容团队维护 prompt 和政策文档，QA 维护 regression trace。若没有清晰契约，任何改动都会互相踩脚。<span class='mono'>create_agent</span> 的装配边界让每个团队知道自己交付什么：工具交付 schema 和观察，RAG 交付 <span class='mono'>Document</span> 和引用，middleware 交付 allow/block 决策，response 交付结构字段。",
            ...
            ("客服 Agent 的失败模式分层", [
                "第一层是知识失败：检索不到政策、引用过期、资料冲突。修复点在索引、切块、metadata、query rewrite 或引用核验。第二层是动作失败：工具参数错、订单不属于用户、副作用重复执行。修复点在工具 schema、runtime context、幂等键和 guard。第三层是表达失败：回答语气不合适、没有澄清、结构字段缺失。修复点在 prompt、<span class='mono'>response_format</span> 和评测样例。",
                "分层能避免错修。若问题是订单工具没有校验 <span class='mono'>user_id</span>，用更强 prompt 说“不要越权”并不够；若问题是政策资料缺失，增加 middleware 也不会凭空产生证据；若问题是 <span class='mono'>structured_response</span> 缺字段，调整 retriever 可能没有帮助。trace 应告诉你失败在哪一层，修复也应落在同一层。",
                "端到端回归测试要覆盖跨层连接。一个案例可以同时验证：RAG 返回政策引用，订单工具返回状态，middleware 阻止直接退款，最终 response 指向人工工单。它不是只测一个函数，而是证明组件之间的合同仍然成立。",
            ]),

            ("从课程零件到生产清单", [
                "把 Capstone 落到生产前，可以把前面课程转成一张上线清单：消息系统负责记录用户与工具观察；聊天模型通过统一接口接入 provider；工具有 schema、幂等和权限；prompt 有模板和版本；RAG 有 <span class='mono'>Document</span>、metadata、引用和拒答；LangGraph 有状态、checkpoint 和控制边；middleware 有 guard 和审计；测试有 fake trace 和真实评测。每一项都对应本书前面的一课。",
                "这张清单能暴露缺口。如果没有引用核验，RAG 只是在 prompt 里塞资料；如果没有 runtime context，权限靠模型猜；如果没有 <span class='mono'>structured_response</span>，后端只能解析自由文本；如果没有 regression trace，重构后不知道防线是否还在。Capstone 的价值是把这些缺口集中展示，而不是假装 demo 已经等于生产。",
            ...
            ("Capstone 演进复盘", [
                ...
                "复盘时可以沿 Capstone 的装配表逐项检查：prompt 是否仍表达最新政策，RAG 引用是否覆盖新增条款，工具是否保持幂等和权限边界，middleware 是否记录拒绝理由，<span class='mono'>structured_response</span> 是否满足 UI，fake trace 是否覆盖最近事故。每一项都有负责人和检查命令，系统才不会随着功能增加而变成黑箱。",
            ]),
        ],
        "pitfalls": [
        ...
        "points": [
            ...
            "Prompt、RAG、工具、middleware、context、<span class='mono'>response_format</span> 各有职责。",
