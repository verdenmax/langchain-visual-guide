...
QUIZZES = {
    ...
    "11-chat-internals.html": {
        "mcq": [
            {
                "q": "在 <code>BaseChatModel</code> 调用链中，为什么要先执行输入标准化，再进入具体 provider adapter？",
                "opts": [
                    "为了让所有请求都变成 OpenAI payload",
                    "为了绕过回调和缓存",
                    "为了让业务代码读取 provider 原始响应",
                    "为了把字符串、PromptValue、message-like 等输入统一成标准消息视图，让 provider wrapper 只负责厂商翻译",
                ],
                "answer": 3,
                "why": "输入标准化把上层宽松输入收束到 BaseMessage/PromptValue 中间表示；ChatOpenAI、ChatAnthropic 等适配器再把它翻译成各自 payload。",
            },
            {
                "q": "<code>ChatGeneration</code> 在聊天模型内部最准确的角色是什么？",
                "opts": [
                    "一个只用于保存 prompt 模板的对象",
                    "一个聊天候选生成结果容器，封装 AIMessage、generation_info 等归一化信息",
                    "一个向量检索器",
                    "一个 provider SDK 客户端",
                ],
                "answer": 1,
                "why": "provider 响应归一后进入 ChatResult/ChatGeneration；invoke 通常取第一条 generation 的 message 返回给上层。",
            },
            {
                "q": "业务代码最不应该做哪件事？",
                "opts": [
                    "读取 <code>AIMessage.content</code>",
                    "使用 <code>usage_metadata</code> 做成本统计",
                    "在核心业务流程里直接依赖某个 provider raw response 的私有字段",
                    "在排障日志里保留 response metadata",
                ],
                "answer": 2,
                "why": "provider 私有字段易变，应收口在 adapter 或排障工具中；业务流程优先依赖标准消息和标准 metadata。",
            },
        ],
        "open": [
            "画出一次 <code>model.invoke</code> 的链路：输入标准化、config/callback、provider payload、raw response、ChatGeneration、AIMessage，每步写出主要输入输出。",
            "如果模型返回了工具调用但 Agent 没有执行工具，你会如何区分是 provider payload 没绑定工具、raw response 没有 tool call，还是响应归一丢失了 <code>tool_calls</code>？",
        ],
    },
    "12-tool-internals.html": {
        "mcq": [
            {
                "q": "为什么说工具的 schema 生成和工具执行是两件事？",
                "opts": [
                    "schema 生成只给模型说明名称、描述和参数契约；只有模型返回 tool_calls 后，ToolNode 才校验参数并执行函数",
                    "schema 生成会自动执行所有工具",
                    "工具执行只发生在 provider 训练阶段",
                    "二者没有区别，只是命名不同",
                ],
                "answer": 0,
                "why": "去程是工具说明，回程才是执行。混淆二者会导致绑定工具时产生副作用或无法定位错误。",
            },
            {
                "q": "<code>args_schema</code> 的运行时价值是什么？",
                "opts": [
                    "只让文档更漂亮",
                    "只决定工具名称",
                    "用 Pydantic 在执行前校验模型生成的 args，拦住缺字段、类型错和约束违背",
                    "替代所有权限检查",
                ],
                "answer": 2,
                "why": "args_schema 既能生成模型可见 schema，也能在执行前验证 JSON 参数；但权限、幂等和业务规则仍需额外处理。",
            },
            {
                "q": "<code>ToolMessage.tool_call_id</code> 为什么必须保留？",
                "opts": [
                    "为了让消息更长",
                    "因为 provider 不允许 content 字段",
                    "为了替代工具返回值",
                    "为了在并行、多工具或重试场景下把工具结果可靠配回原始 tool_call",
                ],
                "answer": 3,
                "why": "按顺序匹配在并发和部分失败时不可靠；tool_call_id 是请求与观察的因果主键。",
            },
        ],
        "open": [
            "为一个有副作用的 <code>refund_order</code> 工具写出 schema 生成、Pydantic 校验、权限检查、执行、ToolMessage 回填五个阶段的职责。",
            "选择三类工具错误：参数格式错、权限不足、外部服务超时。分别说明哪些适合回填 ToolMessage，哪些应抛给应用层或触发人审。",
        ],
    },
    ...
    "21-langchain-vs-autogen.html": {
        "mcq": [
            {
                "q": "横向比较 LangChain/LangGraph、AutoGen、CrewAI、LlamaIndex 等生态时，最可靠的切入点是什么？",
                "opts": [
                    "比较谁的名字更常见",
                    "只看 GitHub star 数",
                    "识别默认心智模型：图/状态、actor 对话、角色任务、数据/RAG 等分别让什么更顺手",
                    "直接断言某个框架已经过时",
                ],
                "answer": 2,
                "why": "功能清单常常都能覆盖，默认抽象才决定开发体验、观测方式和测试边界。",
            },
            {
                "q": "为什么本课避免对外部框架写“当前一定更活跃/更落后”的硬断言？",
                "opts": [
                    "因为这些结论会随版本、团队和社区变化，需要按具体时间和证据核验",
                    "因为生态比较完全没有意义",
                    "因为所有框架 API 永远不变",
                    "因为 LangGraph 不能做多 Agent",
                ],
                "answer": 0,
                "why": "维护状态、API 和社区实践变化很快；课程应比较可迁移的心智模型，并提醒读者核验版本。",
            },
            {
                "q": "如果项目主要难点是严格状态、checkpoint、人审和可回放 trace，哪种默认重心通常更贴近？",
                "opts": [
                    "只用角色描述，不设计状态",
                    "图/状态式编排，例如 LangGraph 的 StateGraph 与 Pregel 运行时",
                    "只让多个 agent 自由聊天，没有终止规则",
                    "只比较模型参数大小",
                ],
                "answer": 1,
                "why": "显式 state、节点、边、checkpoint 和 trace 更适合可控工作流；多 agent 对话也可实现，但需要额外治理。",
            },
        ],
        "open": [
            "为一个“调研并写报告”的任务分别用图/状态、actor 对话、角色 crew、数据/RAG 四种心智模型描述入口，并指出各自最自然和最吃力的部分。",
            "设计一个生态选型矩阵：列出状态控制、数据/RAG、多角色协作、观测、团队熟悉度五个维度，并说明你会如何记录版本证据。",
        ],
    },
    "22-ai-stack.html": {
        "mcq": [
            {
                "q": "MCP、A2A 与 LangChain/LangGraph framework API 的核心区别是什么？",
                "opts": [
                    "MCP 更偏工具/资源/提示等能力连接，A2A 更偏 agent 间任务协作，framework API 负责应用进程内编排和状态",
                    "三者都是同一个 Python 类",
                    "A2A 只用于向量检索",
                    "framework API 会自动替代所有外部协议",
                ],
                "answer": 0,
                "why": "分清工具协议、agent-to-agent 协议和框架 API，才能设计适配层和责任边界。",
            },
            {
                "q": "为什么协议字段不应散落在业务图的每个节点里？",
                "opts": [
                    "因为业务图不能调用外部工具",
                    "因为字段名太短",
                    "因为通过 adapter 映射到 BaseTool、ToolMessage 或 state，能在协议演进时局部修改",
                    "因为协议永远不会变化",
                ],
                "answer": 2,
                "why": "adapter 是抗演进缓冲垫；协议升级时主要修改 adapter，而不是重写业务流程。",
            },
            {
                "q": "接入一个外部 MCP 工具后，应用层仍必须负责什么？",
                "opts": [
                    "训练底层大模型",
                    "删除所有工具 schema",
                    "关闭所有日志",
                    "认证授权、租户隔离、审计、脱敏、危险动作确认和降级策略",
                ],
                "answer": 3,
                "why": "协议标准化连接方式，但不替代业务安全、权限、观测和治理责任。",
            },
        ],
        "open": [
            "画出一次请求经过产品层、LangGraph 编排层、MCP 工具边界、A2A 外部 agent 边界、模型/数据底座的 trace，并写出每层失败时如何定位。",
            "为一个 MCP 日历工具写 adapter 设计：哪些协议字段收口，映射成哪个 BaseTool schema，ToolMessage 返回什么，超时和权限不足如何处理。",
        ],
    },
    "23-learning-map.html": {
        "mcq": [
            {
                "q": "学完本教程后，选择下一个学习方向最应该依据什么？",
                "opts": [
                    "真实项目症状与相邻层：慢/贵看推理，找不准看向量检索，不可控看评测观测，外部集成看协议",
                    "哪个名词今天最热",
                    "跳过应用层直接学硬件",
                    "只收藏链接不做实验",
                ],
                "answer": 0,
                "why": "学习路线应由问题驱动，并通过小实验形成反馈，而不是被热点牵着走。",
            },
            {
                "q": "向量数据库/RAG 支线的小实验最应记录什么？",
                "opts": [
                    "只记录最终答案好不好听",
                    "只记录模型名称",
                    "检索到的 Document id、score、metadata、片段、rerank 后顺序和最终引用",
                    "只记录 UI 截图",
                ],
                "answer": 2,
                "why": "RAG 排障要沿检索证据链定位，不能只看最终文本。",
            },
            {
                "q": "评测与可观测学习的目标是什么？",
                "opts": [
                    "让所有回答都变长",
                    "把 prompt 调整、工具变化、检索变化和模型升级的影响变成可重复质量信号",
                    "替代所有人工判断",
                    "隐藏 trace 细节",
                ],
                "answer": 1,
                "why": "固定数据集、trace 断言、成本延迟指标和人工样本共同帮助判断是否退化。",
            },
        ],
        "open": [
            "根据你当前项目的一个症状，选择推理引擎、向量数据库、评测观测、协议、贡献中的一个方向，写出四周学习 sprint。",
            "为一次学习产出定义可复查 artifact：脚本、数据集、trace、测试或 PR，并说明它如何反哺现有系统。",
        ],
    },
    "27-glossary.html": {
        "mcq": [
            {
                "q": "C-level 术语表最重要的用途是什么？",
                "opts": [
                    "作为概念索引、页面索引和源码锚点表，把术语连接回课程、符号和项目测试",
                    "替代所有课程正文",
                    "只按旧课号背定义",
                    "只列外部生态名字",
                ],
                "answer": 0,
                "why": "术语表的目标是帮助查词、跳课、读源码和映射项目，而不是孤立背单词。",
            },
            {
                "q": "为什么源码锚点推荐“文件 + 符号名”而不是固定行号？",
                "opts": [
                    "因为源码没有文件名",
                    "因为行号更容易随上游修改漂移，符号名更适合长期复查",
                    "因为符号名不能搜索",
                    "因为术语表不需要可验证性",
                ],
                "answer": 1,
                "why": "文件 + 符号名兼顾可复查和抗漂移；行号在频繁演进的上游仓库里很容易过期。",
            },
            {
                "q": "查到 <code>AgentMiddleware</code> 后，最健康的复习路径是什么？",
                "opts": [
                    "只记住这个英文名",
                    "直接复制任意旧代码",
                    "把它当成向量数据库接口",
                    "回到对应课程理解生命周期，再看源码锚点，最后映射到本地 Agent 的可测试策略",
                ],
                "answer": 3,
                "why": "术语学习要经过课程上下文、源码入口和本地项目映射，最终能写测试验证行为。",
            },
        ],
        "open": [
            "选择术语表中的三个符号：一个 core 概念、一个 LangGraph 概念、一个 Agent/RAG 概念。分别写出对应页面、源码锚点和你项目里的类比对象。",
            "如果未来课程新增页面或上游源码移动，你会如何检查术语表没有旧标题、旧课号或旧路径残留？",
        ],
    },
    ...
    "30-langgraph-reducers-channels.html": {
        "mcq": [
            ...
            },
            {
                ...
                "answer": 1,
                "why": "并行 fan-in 需要明确合并法则。没有 reducer 时，覆盖或冲突都可能让结果不可解释。",
            },
        ],
        "open": [
