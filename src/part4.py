...
<div class="card detail">
  ...
  <ul>
    <li><strong>看真实请求</strong>：给 <span class="mono">invoke</span> 传一个打印型回调（<span class="mono">config={"callbacks":[...]}</span>，见第 16 课），
...
<div class="card key">
  ...
  <ul>
    ...
    <li>这节旧工程章节先作为迁移参考保留；后续扩展后的工程实践章节会重新梳理这些读源码、测试、调试与贡献流程。</li>
  </ul>
</div>
"""
