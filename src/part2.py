...
<div class="card macro">
  ...
  <ul>
    ...
    <li><strong>tool_calls 是 Agent 的引擎</strong>：模型不直接执行工具，而是在 AIMessage 里"请求"调用，
      由你的程序去执行，再把结果包成 ToolMessage 喂回去。这就是 Agent 循环的本质（详见 Agent 入门与 Agent 内部课程）。</li>
...
<div class="cols">
  <div class="col">
    ...
    <p>流式输出时，模型不是一次给完整 <span class="inline">AIMessage</span>，而是一串
      <span class="inline">AIMessageChunk</span>。它们能用 <span class="inline">+</span> 累加成完整消息（Streaming 与 Callbacks 课详解）。</p>
...
agent = create_agent(model, tools, middleware=[SummarizationMiddleware(model)])</pre>
    ...
    <div class="qa">
      ...
      <div class="a"><span class="mono">SummarizationMiddleware</span> → <span class="mono">agents/middleware/summarization.py</span>；
        ...
        <span class="mono">checkpointer</span> 是 <span class="mono">create_agent</span> 的参数（持久化由 LangGraph 提供，见 compile 与 Runtime / Agent 内部相关内容）。</div>
    </div>
  </div>
</details>
...
<p>拿到模型后，有三种跑法。它们其实来自更底层的 <strong>Runnable 接口</strong>（Runnable 协议课会揭晓）：</p>
...
a*      → 上面三个的异步版（FastAPI 等异步框架里用）</pre>
    ...
    <div class="qa">
      <div class="q">❓ 什么时候用异步版（a*）</div>
      <div class="a">当代码跑在 <strong>asyncio 事件循环</strong>里（FastAPI、aiohttp、聊天机器人等），用
        <span class="mono">await model.ainvoke(...)</span> 能在等模型返回时<strong>不阻塞</strong>其它请求，单机扛更多并发；
        普通脚本或同步框架用同步版即可。<pre class="code"><span class="kw">resp</span> = <span class="kw">await</span> model.ainvoke(<span class="st">"你好"</span>)   <span class="cm"># 必须在 async def 内</span></pre></div>
    </div>
    <div class="qa">
    ...
    <div class="qa">
      <div class="q">✅ 优点</div>
      <div class="a">它们其实都来自底层 <strong>Runnable</strong> 接口（Runnable 协议课），所以<strong>任何</strong> LangChain 组件
...
<h2>🎚️ 进阶专题：参数、缓存、重试、限流</h2>
<p>同一个模型，行为可以被大量参数和"包装器"调节：控制随机性、固定默认参数、省钱（缓存）、
抗抖动（重试）、防超额（限流）。这些是把 demo 变成生产应用的关键开关。</p>

<table class="t">
  <tr><th>目的</th><th>手段</th></tr>
  <tr><td>控制输出（随机性/长度）</td><td class="mono">init 时传 temperature / max_tokens 等</td></tr>
  <tr><td>固定默认调用参数</td><td class="mono">model.bind(stop=..., ...)</td></tr>
  <tr><td>重复问题不重复请求</td><td class="mono">缓存（cache）</td></tr>
  <tr><td>偶发错误自动重试</td><td class="mono">model.with_retry()</td></tr>
  <tr><td>控制请求速率，防 429</td><td class="mono">InMemoryRateLimiter</td></tr>
</table>

<h2>🔍 调用控制 · 深入理解</h2>
<p class="acc-intro" style="color:var(--muted);font-size:.92rem">展开下面的卡片，逐个吃透每个开关。</p>

<details class="accordion">
  <summary><span class="badge-num">1</span> 调输出：temperature 等参数 + .bind 固定默认 <span class="hint">点击展开详解</span></summary>
  <div class="acc-body">
    <div class="qa">
      <div class="q">🧪 两种设定方式</div>
      <div class="a">
<pre class="code"><span class="cm"># 1) 实例化时设定（影响这个模型的所有调用）</span>
model = init_chat_model(<span class="st">"openai:gpt-5.1"</span>, temperature=0)  <span class="cm"># 0=更确定</span>

<span class="cm"># 2) .bind 固定某些默认调用参数，返回一个新 Runnable</span>
strict = model.bind(stop=[<span class="st">"\n\n"</span>])   <span class="cm"># 之后每次调用都带上这个 stop</span></pre>
      </div>
    </div>
    <div class="qa">
      <div class="q">❓ temperature 是什么 / bind 有何用</div>
      <div class="a"><span class="mono">temperature</span> 控制随机性：低=稳定可复现（适合抽取/分类），高=有创意（适合头脑风暴）。
        <span class="mono">.bind</span> 把"每次都要传的参数"<strong>预先绑死</strong>，省得每次调用重复写，且产物仍是 Runnable 可继续组合。</div>
    </div>
    <div class="qa">
      <div class="q">✅ 优点</div>
      <div class="a">实例级设定影响全局、bind 级设定可派生多个"预设版本"模型（如一个严格版、一个发散版），互不影响。</div>
    </div>
  </div>
</details>

<details class="accordion">
  <summary><span class="badge-num">2</span> 省钱与抗抖动：缓存 + with_retry <span class="hint">点击展开详解</span></summary>
  <div class="acc-body">
    <div class="qa">
      <div class="q">🧪 示例</div>
      <div class="a">
<pre class="code"><span class="cm"># 缓存：相同输入直接返回上次结果，不再请求（省钱省时间）</span>
<span class="kw">from</span> langchain_core.globals <span class="kw">import</span> set_llm_cache
<span class="kw">from</span> langchain_core.caches <span class="kw">import</span> InMemoryCache
set_llm_cache(InMemoryCache())

<span class="cm"># 重试：偶发错误（429/超时）自动重试，返回一个带重试的 Runnable</span>
robust = model.with_retry(stop_after_attempt=3)</pre>
      </div>
    </div>
    <div class="qa">
      <div class="q">❓ 它们解决什么</div>
      <div class="a">缓存命中时<strong>根本不发请求</strong>（呼应聊天模型内部课的 <span class="mono">_generate_with_cache</span>），重复问答零成本；
        <span class="mono">with_retry</span> 是 Runnable 通用能力，用指数退避吸收瞬时故障。</div>
    </div>
    <div class="qa">
      <div class="q">✅ 优点</div>
      <div class="a">缓存与重试都是"包一层"就生效，业务代码不变；<span class="mono">with_retry</span> 对任何 Runnable（不止模型）都适用。</div>
    </div>
  </div>
</details>

<details class="accordion">
  <summary><span class="badge-num">3</span> 防超额：用限流器控制请求速率 <span class="hint">点击展开详解</span></summary>
  <div class="acc-body">
    <div class="qa">
      <div class="q">🧪 示例</div>
      <div class="a">
<pre class="code"><span class="kw">from</span> langchain_core.rate_limiters <span class="kw">import</span> InMemoryRateLimiter

limiter = InMemoryRateLimiter(requests_per_second=2)   <span class="cm"># 每秒最多 2 次</span>
model = init_chat_model(<span class="st">"openai:gpt-5.1"</span>, rate_limiter=limiter)</pre>
      </div>
    </div>
    <div class="qa">
      <div class="q">❓ 为什么必要</div>
      <div class="a">厂商有 RPM/TPM 配额，<span class="mono">batch</span> 或高并发场景很容易触发 <strong>429 Too Many Requests</strong>。
        限流器在客户端<strong>主动节流</strong>，从源头避免被拒。</div>
    </div>
    <div class="qa">
      <div class="q">✅ 代码对应 & 🔀 其他方案</div>
      <div class="a"><span class="mono">InMemoryRateLimiter</span> 在 <span class="mono">core/rate_limiters.py</span>（进程内限流）。
        其他方案：分布式部署时用基于 Redis 的限流；或在网关层统一限流。</div>
    </div>
  </div>
</details>
...
<span class="cm"># → {"raw": AIMessage(...), "parsed": Weather(...), "parsing_error": None}</span></pre>
    ...
    <div class="qa">
      ...
      <div class="a"><span class="mono">response_format</span>（Agent 入门课）底层用
...
<div class="card key">
  ...
  <ul>
    ...
    <li>行为可调：<strong>temperature/bind</strong>（控输出）、<strong>缓存</strong>（省钱）、<strong>with_retry</strong>（抗抖动）、<strong>限流器</strong>（防 429）。</li>
...
<div class="card macro">
  ...
  Agent 入门课的 <span class="mono">create_agent</span> 就是把这整个循环<strong>自动化</strong>。
  也就是说：<strong>工具 + 这个循环 = Agent</strong>。
</div>

<h3>手写一次完整往返（可跑）</h3>
<pre class="code"><span class="kw">from</span> langchain.chat_models <span class="kw">import</span> init_chat_model
<span class="kw">from</span> langchain_core.messages <span class="kw">import</span> HumanMessage, ToolMessage

model = init_chat_model(<span class="st">"openai:gpt-5.1"</span>).bind_tools([get_weather])

msgs = [HumanMessage(<span class="st">"北京天气怎么样？"</span>)]
ai = model.invoke(msgs)                    <span class="cm"># ① 模型请求调用工具</span>
msgs.append(ai)

<span class="kw">for</span> call <span class="kw">in</span> ai.tool_calls:                  <span class="cm"># ② 你来执行（可能不止一个）</span>
    result = get_weather.invoke(call[<span class="st">"args"</span>])
    msgs.append(ToolMessage(result, tool_call_id=call[<span class="st">"id"</span>]))  <span class="cm"># ③ 回传，靠 id 配对</span>

final = model.invoke(msgs)                 <span class="cm"># ④ 再问 → 自然语言答复</span>
<span class="nb">print</span>(final.content)
</pre>
<p style="color:var(--muted);font-size:.9rem">这正是 <span class="mono">create_agent</span> 内部循环的<strong>手写版</strong>——把它自动化（再加多轮与错误处理）就是一个 Agent。</p>
...
<h2>🧰 进阶专题：让工具更强大、更安全</h2>
<p>真实工具不只是"输入→输出"。它们常需要：出错时优雅处理、读取 Agent 的运行时状态、
危险操作前人审、工具太多时先筛选。这些大多在 Agent / 工具节点层提供。</p>

<table class="t">
  <tr><th>需求</th><th>机制</th></tr>
  <tr><td>工具抛异常不崩溃</td><td class="mono">ToolNode 错误处理 · ToolRetryMiddleware · status="error"</td></tr>
  <tr><td>工具读取 Agent 状态</td><td class="mono">InjectedState · InjectedToolCallId</td></tr>
  <tr><td>危险工具先人审</td><td class="mono">HumanInTheLoopMiddleware</td></tr>
  <tr><td>工具太多先筛一遍</td><td class="mono">LLMToolSelectorMiddleware</td></tr>
</table>

<h2>🔍 工具进阶 · 深入理解</h2>
<p class="acc-intro" style="color:var(--muted);font-size:.92rem">展开下面的卡片，逐个吃透进阶能力。</p>

<details class="accordion">
  <summary><span class="badge-num">1</span> 工具出错了，Agent 怎么不崩 <span class="hint">点击展开详解</span></summary>
  <div class="acc-body">
    <div class="qa">
      <div class="q">🧪 错误被转成消息，而非异常</div>
      <div class="a">在 Agent 里，工具抛异常时 <span class="mono">ToolNode</span> 会把它<strong>捕获并转成一条 <span class="mono">ToolMessage</span></strong>
        （<span class="mono">status="error"</span>），让模型"看到错误"并自行调整：
<pre class="code">ToolMessage(content=<span class="st">"Error: 城市不存在"</span>, status=<span class="st">"error"</span>, tool_call_id=...)
<span class="cm"># 模型读到后可改参数重试，或换个工具，而不是整个程序崩掉</span></pre>
      </div>
    </div>
    <div class="qa">
      <div class="q">❓ 为什么这样设计</div>
      <div class="a">Agent 是自动循环，单个工具失败不该让整条流程挂掉。把错误<strong>反馈给模型</strong>，
        它往往能自我纠正（如换个查询）。要"自动重试"则叠加 <span class="mono">ToolRetryMiddleware</span>。</div>
    </div>
    <div class="qa">
      <div class="q">✅ 优点</div>
      <div class="a">把"异常"变成"对话信号"，让 Agent 具备一定的自愈能力；是否捕获/如何捕获可在 ToolNode 配置。</div>
    </div>
  </div>
</details>

<details class="accordion">
  <summary><span class="badge-num">2</span> 让工具读取 Agent 状态：InjectedState <span class="hint">点击展开详解</span></summary>
  <div class="acc-body">
    <div class="qa">
      <div class="q">🧪 示例</div>
      <div class="a">
<pre class="code"><span class="kw">from</span> typing <span class="kw">import</span> Annotated
<span class="kw">from</span> langchain.tools <span class="kw">import</span> InjectedState

<span class="nb">@tool</span>
<span class="kw">def</span> <span class="fn">summarize_history</span>(state: Annotated[dict, InjectedState]) -> str:
    <span class="st">&quot;&quot;&quot;总结当前对话。&quot;&quot;&quot;</span>
    msgs = state[<span class="st">"messages"</span>]   <span class="cm"># 工具能拿到 Agent 状态</span>
    ...</pre>
      </div>
    </div>
    <div class="qa">
      <div class="q">❓ 为什么必要 / 妙在哪</div>
      <div class="a">有些工具需要"对话上下文"或"运行时数据"才能工作，但这些<strong>不该由模型来填参数</strong>
        （模型也填不对）。<span class="mono">InjectedState</span> 标记的参数会被<strong>框架自动注入</strong>，
        且<strong>不会出现在给模型看的 schema 里</strong>——模型根本不知道它存在。
        类似地 <span class="mono">InjectedToolCallId</span> 注入当前调用 id。</div>
    </div>
    <div class="qa">
      <div class="q">✅ 代码对应</div>
      <div class="a"><span class="mono">InjectedState / InjectedToolCallId</span> 都从 <span class="mono">langchain.tools</span> 统一导出
        （<span class="mono">InjectedState</span> 来自 <span class="mono">langgraph.prebuilt</span>，<span class="mono">InjectedToolCallId</span> 来自 <span class="mono">langchain_core.tools</span>）。</div>
    </div>
  </div>
</details>

<details class="accordion">
  <summary><span class="badge-num">3</span> 危险工具人审 + 工具太多先筛 <span class="hint">点击展开详解</span></summary>
  <div class="acc-body">
    <div class="qa">
      <div class="q">🧪 两个中间件</div>
      <div class="a">
<pre class="code"><span class="kw">from</span> langchain.agents.middleware <span class="kw">import</span> (
    HumanInTheLoopMiddleware,     <span class="cm"># 执行敏感工具前暂停等批准</span>
    LLMToolSelectorMiddleware,    <span class="cm"># 工具很多时，先让模型挑出相关子集</span>
)
agent = create_agent(model, tools=large_toolset, middleware=[
    ...
    <div class="qa">
      <div class="q">❓ 各自解决什么</div>
      <div class="a"><strong>工具太多</strong>（几十上百个）会撑爆上下文、降低选择准确率，
        <span class="mono">LLMToolSelectorMiddleware</span> 先按当前任务筛出相关工具再交给模型。
        <strong>危险工具</strong>则用 <span class="mono">HumanInTheLoopMiddleware</span> 加人审闸门。</div>
    </div>
    <div class="qa">
      <div class="q">✅ 优点</div>
...
agent = create_agent(model, tools=toolkit.get_tools())   <span class="cm"># 一次性给一组工具</span></pre>
    ...
    <div class="qa">
      ...
      <div class="a">它是<strong>所有"注入参数"的基类</strong>（<span class="mono">tools/base.py</span>）：工具 Tools 课的
...
<div class="card key">
  ...
  <ul>
    ...
    <li>进阶：错误转消息（自愈）、<span class="mono">InjectedState</span>（读状态不暴露给模型）、人审、工具筛选。</li>
...
<div class="card detail">
  ...
  <ul>
    ...
    *,
    system_prompt=None,
    middleware=(),         <span class="cm"># 可插拔的中间件（Agent 内部课）</span>
...
<p>这张图是 Agent 的灵魂。注意它<strong>就是工具 Tools 课里的工具往返，只是自动重复</strong>，
直到模型不再请求工具为止：</p>
...
<div class="card macro">
  ...
  <ul>
    ...
    <li>底层其实是一张 <strong>LangGraph 状态图</strong>（节点："model" 与 "tools"）——这是 Agent 内部课的内容，现在只需理解它在转一个循环。</li>
  </ul>
</div>
...
<span class="cm"># Agent 跑完循环后，给你一个 Answer 对象，而不只是文本</span></pre>
    ...
    <div class="qa">
      ...
      <div class="a">聊天模型课的 <span class="mono">with_structured_output</span> 约束<strong>单次模型调用</strong>的输出；
...
<details class="accordion">
  <summary><span class="badge-num">3</span> middleware：在循环里插钩子（预告 Agent 内部课） <span class="hint">点击展开详解</span></summary>
  <div class="acc-body">
    <div class="qa">
    ...
    <div class="qa">
      <div class="q">✅ 优点 & 🔀 其他方案</div>
      <div class="a">优点：行为可插拔，核心循环保持稳定（Agent 内部课看它如何映射到 LangGraph 节点）。
        其他方案：用回调（callbacks，Streaming 与 Callbacks 课）做观测，但 middleware 能真正<strong>改变</strong>流程，回调主要用于<strong>观察</strong>。</div>
    </div>
  </div>
</details>

<h2>🎛️ 进阶专题：如何"驯服"一个 Agent</h2>
<p>自动循环很强，但生产环境需要给它装上<strong>刹车与护栏</strong>：限制循环次数、关键操作要人审、
出错能重试或降级。这些大多通过<strong>中间件（middleware）</strong>声明式装上，全部来自 <span class="inline">langchain.agents.middleware</span>。</p>

<table class="t">
  <tr><th>需求</th><th>中间件 / 机制</th></tr>
  <tr><td>防止无限循环 / 失控</td><td class="mono">ModelCallLimitMiddleware · ToolCallLimitMiddleware · recursion_limit</td></tr>
  <tr><td>关键操作要人批准</td><td class="mono">HumanInTheLoopMiddleware</td></tr>
  <tr><td>出错自动重试</td><td class="mono">ModelRetryMiddleware · ToolRetryMiddleware</td></tr>
  <tr><td>主模型挂了降级备用</td><td class="mono">ModelFallbackMiddleware</td></tr>
  <tr><td>工具太多先筛一遍</td><td class="mono">LLMToolSelectorMiddleware</td></tr>
</table>

<h2>🔍 Agent 控制 · 深入理解</h2>
<p class="acc-intro" style="color:var(--muted);font-size:.92rem">展开下面的卡片，逐个吃透关键护栏。</p>

<details class="accordion">
  <summary><span class="badge-num">1</span> 防止失控：循环与调用次数上限 <span class="hint">点击展开详解</span></summary>
  <div class="acc-body">
    <div class="qa">
      <div class="q">🧪 两道防线</div>
      <div class="a">
<pre class="code"><span class="kw">from</span> langchain.agents.middleware <span class="kw">import</span> (
...
agent = create_agent(model, tools, middleware=[
    ModelCallLimitMiddleware(run_limit=8),   <span class="cm"># 本次运行最多调 8 次模型（思考轮数）</span>
    ToolCallLimitMiddleware(run_limit=3),    <span class="cm"># 本次运行最多调 3 次工具（防失控刷工具）</span>
])
<span class="cm"># 硬保护：图的递归上限（兜底，超过直接报错）</span>
agent.invoke(input, config={<span class="st">"recursion_limit"</span>: 25})</pre>
      </div>
    </div>
    <div class="qa">
      <div class="q">❓ Model 上限 vs Tool 上限</div>
      <div class="a"><span class="mono">ModelCallLimitMiddleware</span> 限"<strong>总共思考几轮</strong>"（模型被调用次数）；
        <span class="mono">ToolCallLimitMiddleware</span> 限"<strong>某个/某类工具最多被调几次</strong>"，防止模型对同一工具反复刷。
        二者可叠加，分别防"想太久"和"刷某工具"。</div>
    </div>
    <div class="qa">
      <div class="q">❓ 为什么必要</div>
      <div class="a">模型可能陷入"反复调同一个工具"的死循环，既烧钱又卡死。
        <span class="mono">recursion_limit</span> 是 LangGraph 的兜底硬上限；<span class="mono">ModelCallLimit / ToolCallLimit</span>
        则是更贴业务的软上限，可控制"最多思考几轮、某工具最多调几次"。</div>
    </div>
    <div class="qa">
      <div class="q">✅ 优点</div>
      <div class="a">两层防线：软上限给出友好行为（如到达上限就收尾回答），硬上限保证绝不无限跑。</div>
    </div>
  </div>
</details>

<details class="accordion">
  <summary><span class="badge-num">2</span> 人在回路（HITL）：危险操作前先批准 <span class="hint">点击展开详解</span></summary>
  <div class="acc-body">
    <div class="qa">
      <div class="q">🧪 示例</div>
      <div class="a">
<pre class="code"><span class="kw">from</span> langchain.agents.middleware <span class="kw">import</span> HumanInTheLoopMiddleware

agent = create_agent(model, tools=[delete_database], middleware=[
    HumanInTheLoopMiddleware(...),   <span class="cm"># 调用敏感工具前暂停，等人确认</span>
], checkpointer=saver)               <span class="cm"># HITL 需要可暂停/恢复，故要 checkpointer</span></pre>
      </div>
    </div>
    <div class="qa">
      <div class="q">❓ 为什么必要</div>
      <div class="a">有些工具是<strong>不可逆的高风险操作</strong>（删数据、转账、发邮件）。让 Agent 在执行前<strong>暂停等人点头</strong>，
        是生产环境的安全底线。它依赖图的"中断/恢复"能力（Agent 内部 / LangGraph 持久化相关内容）。</div>
    </div>
    <div class="qa">
      <div class="q">✅ 优点</div>
      <div class="a">把"自动"与"可控"结合：日常自动跑，关键节点交回人类决策，且暂停期间状态被持久化、可随时恢复。</div>
    </div>
  </div>
</details>

<details class="accordion">
  <summary><span class="badge-num">3</span> 健壮性：重试与模型降级 <span class="hint">点击展开详解</span></summary>
  <div class="acc-body">
    <div class="qa">
      <div class="q">🧪 示例</div>
      <div class="a">
...
agent = create_agent(primary_model, tools, middleware=[
    ToolRetryMiddleware(max_retries=2),        <span class="cm"># 工具抖动自动重试</span>
    ModelFallbackMiddleware(backup_model),     <span class="cm"># 主模型失败→切备用</span>
])</pre>
      </div>
    </div>
    <div class="qa">
      <div class="q">❓ 为什么必要</div>
      <div class="a">网络抖动、限流 429、厂商宕机都会偶发。<span class="mono">Retry</span> 吸收瞬时错误，
        <span class="mono">Fallback</span> 在主模型彻底不可用时切到备用，保证服务连续。</div>
    </div>
    <div class="qa">
      <div class="q">✅ 优点 & 🔀 其他方案</div>
      <div class="a">优点：健壮性以"装中间件"的方式声明式获得，核心逻辑不变。
        其他方案：在单次模型调用层用 <span class="mono">model.with_retry()</span>（聊天模型课），但 Agent 级中间件覆盖整个循环更全面。</div>
    </div>
  </div>
</details>
...
<div class="card key">
  ...
  <ul>
    ...
    <li>循环判据：<strong>AIMessage 里还有没有 tool_calls</strong>。这把消息、聊天模型、工具三课串成了闭环。</li>
