...
<span class="cm"># → {"raw": AIMessage(...), "parsed": Weather(...), "parsing_error": None}</span></pre>
    ...
    <div class="qa">
      ...
      <div class="a"><span class="mono">response_format</span>（第 7 课）底层用
        <span class="mono">ToolStrategy</span> / <span class="mono">ProviderStrategy</span> /
        <span class="mono">AutoStrategy</span>（自动选）三种策略都在 <span class="mono">structured_output.py</span>。核心签名在 <span class="mono">chat_models.py</span>。</div>
    </div>
  </div>
</details>
...
agent = create_agent(model, tools=toolkit.get_tools())   <span class="cm"># 一次性给一组工具</span></pre>
    ...
    <div class="qa">
      ...
      <div class="a">它是<strong>所有"注入参数"的基类</strong>（<span class="mono">tools/base.py</span>）：本课的
        <span class="mono">InjectedState</span>、<span class="mono">InjectedToolCallId</span> 都继承它。
...
agent = create_agent(primary_model, tools, middleware=[
    ...
    <div class="qa">
      ...
      <div class="a">优点：健壮性以"装中间件"的方式声明式获得，核心逻辑不变。
        其他方案：在单次模型调用层用 <span class="mono">model.with_retry()</span>（第 5 课），但 Agent 级中间件覆盖整个循环更全面。</div>
    </div>
  </div>
</details>
