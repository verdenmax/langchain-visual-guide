...
<div class="card macro">
  ...
  <ul>
    ...
    <li><strong>tool_calls 是 Agent 的引擎</strong>：模型不直接执行工具，而是在 AIMessage 里"请求"调用，
      由你的程序去执行，再把结果包成 ToolMessage 喂回去。这就是 Agent 循环的本质（第 9、15 课）。</li>
...
<div class="cols">
  <div class="col">
    ...
    <p>流式输出时，模型不是一次给完整 <span class="inline">AIMessage</span>，而是一串
      <span class="inline">AIMessageChunk</span>。它们能用 <span class="inline">+</span> 累加成完整消息（第 16 课详解）。</p>
...
agent = create_agent(model, tools, middleware=[SummarizationMiddleware(model)])</pre>
    ...
    <div class="qa">
      ...
      <div class="a"><span class="mono">SummarizationMiddleware</span> → <span class="mono">agents/middleware/summarization.py</span>；
        ...
        <span class="mono">checkpointer</span> 是 <span class="mono">create_agent</span> 的参数（持久化由 LangGraph 提供，第 15 课）。</div>
    </div>
  </div>
</details>
...
<p>拿到模型后，有三种跑法。它们其实来自更底层的 <strong>Runnable 接口</strong>（第 10 课会揭晓）：</p>
...
a*      → 上面三个的异步版（FastAPI 等异步框架里用）</pre>
    ...
    <div class="qa">
      <div class="q">✅ 优点</div>
      <div class="a">它们其实都来自底层 <strong>Runnable</strong> 接口（第 10 课），所以<strong>任何</strong> LangChain 组件
...
robust = model.with_retry(stop_after_attempt=3)</pre>
    ...
    <div class="qa">
      ...
      <div class="a">缓存命中时<strong>根本不发请求</strong>（呼应第 13 课的 <span class="mono">_generate_with_cache</span>），重复问答零成本；
...
<span class="cm"># → {"raw": AIMessage(...), "parsed": Weather(...), "parsing_error": None}</span></pre>
    ...
    <div class="qa">
      ...
      <div class="a"><span class="mono">response_format</span>（第 9 课）底层用
...
<div class="card macro">
  ...
  第 9 课的 <span class="mono">create_agent</span> 就是把这整个循环<strong>自动化</strong>。
...
agent = create_agent(model, tools=toolkit.get_tools())   <span class="cm"># 一次性给一组工具</span></pre>
    ...
    <div class="qa">
      ...
      <div class="a">它是<strong>所有"注入参数"的基类</strong>（<span class="mono">tools/base.py</span>）：第 8 课的
...
<div class="card detail">
  ...
  <ul>
    ...
    *,
    system_prompt=None,
    middleware=(),         <span class="cm"># 可插拔的中间件（第 15 课）</span>
...
<p>这张图是 Agent 的灵魂。注意它<strong>就是第 8 课那个工具往返，只是自动重复</strong>，
直到模型不再请求工具为止：</p>
...
<div class="card macro">
  ...
  <ul>
    ...
    <li>底层其实是一张 <strong>LangGraph 状态图</strong>（节点："model" 与 "tools"）——这是第 15 课的内容，现在只需理解它在转一个循环。</li>
  </ul>
</div>
...
<span class="cm"># Agent 跑完循环后，给你一个 Answer 对象，而不只是文本</span></pre>
    ...
    <div class="qa">
      ...
      <div class="a">第 7 课的 <span class="mono">with_structured_output</span> 约束<strong>单次模型调用</strong>的输出；
...
<details class="accordion">
  <summary><span class="badge-num">3</span> middleware：在循环里插钩子（预告第 15 课） <span class="hint">点击展开详解</span></summary>
  <div class="acc-body">
    <div class="qa">
    ...
    <div class="qa">
      <div class="q">✅ 优点 & 🔀 其他方案</div>
      <div class="a">优点：行为可插拔，核心循环保持稳定（第 15 课看它如何映射到 LangGraph 节点）。
        其他方案：用回调（callbacks，第 16 课）做观测，但 middleware 能真正<strong>改变</strong>流程，回调主要用于<strong>观察</strong>。</div>
    </div>
  </div>
</details>
...
agent = create_agent(model, tools=[delete_database], middleware=[
    ...
    <div class="qa">
      ...
      <div class="a">有些工具是<strong>不可逆的高风险操作</strong>（删数据、转账、发邮件）。让 Agent 在执行前<strong>暂停等人点头</strong>，
        是生产环境的安全底线。它依赖图的"中断/恢复"能力（第 15 课）。</div>
    </div>
    <div class="qa">
      <div class="q">✅ 优点</div>
...
agent = create_agent(primary_model, tools, middleware=[
    ...
    <div class="qa">
      ...
      <div class="a">优点：健壮性以"装中间件"的方式声明式获得，核心逻辑不变。
        其他方案：在单次模型调用层用 <span class="mono">model.with_retry()</span>（第 7 课），但 Agent 级中间件覆盖整个循环更全面。</div>
    </div>
  </div>
</details>
...
<div class="card key">
  <div class="tag">✅ 本课要点</div>
    ...
    <li>循环判据：<strong>AIMessage 里还有没有 tool_calls</strong>。这把第 6–8 课串成了闭环。</li>
