...
model = init_chat_model(<span class="st">"gpt-5.1"</span>).bind_tools([get_weather])
...
<span class="kw">def</span> <span class="fn">summarize_history</span>(state: Annotated[dict, InjectedState]) -> str:
    ...
    <div class="qa">
      <div class="q">✅ 代码对应</div>
      <div class="a"><span class="mono">InjectedState / InjectedToolCallId</span> 从 <span class="mono">langchain.tools</span> 导出
        （底层来自 <span class="mono">langgraph.prebuilt</span>）。</div>
    </div>
  </div>
</details>
...
agent = create_agent(model, tools, middleware=[
    ModelCallLimitMiddleware(max_calls=8),   <span class="cm"># 整体：最多调 8 次模型（思考轮数）</span>
    ToolCallLimitMiddleware(max_calls=3),    <span class="cm"># 单类工具最多调 3 次（防刷某工具）</span>
])
