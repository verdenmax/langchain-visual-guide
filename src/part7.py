...
第 13 课你看到：<span class="inline">create_agent</span> 把 Agent 循环<strong>编译成一张 LangGraph 状态图</strong>。
...
<div class="card analogy">
  ...
  你在第 8/9 课学的 <strong>LCEL（<span class="mono">Runnable |</span>）像一条直的传送带</strong>——东西一头进、一头出，绝不回头。
...
<div class="card detail">
  ...
  <table class="t" style="margin-top:.6rem">
    ...
    <tr><td>reducer（如 add_messages）</td><td class="mono">graph/message.py · 第 25 课详解</td></tr>
  </table>
</div>
...
<div class="card detail">
  ...
  <span class="mono">CompiledStateGraph</span> <strong>本身就是一个 Runnable</strong>，所以第 8 课关于 Runnable 的一切
  ...
  在第 22 课的全栈图里，LangChain 与 LangGraph 都在 <strong>L7 编排层</strong>。
</div>
...
<details class="accordion">
  ...
  <div class="acc-body">
    ...
    <div class="qa">
      ...
      <div class="a">产物是 <strong>Runnable</strong>，于是 <span class="mono">invoke/stream/batch</span>、回调、<span class="mono">config</span> 全部免费获得（第 8 课）。</div>
    </div>
  </div>
</details>
...
<details class="accordion">
  ...
  <div class="acc-body">
    ...
    <div class="qa">
      ...
      <div class="a">第 25 课会看到：编译时<strong>每个键变成一个 channel</strong>，reducer 就是那个 channel 的"更新函数"。</div>
    </div>
  </div>
</details>
...
<div class="card detail">
  ...
  <ul>
    ...
    <li>步数上限：<span class="mono">tick()</span> 开头检查 <span class="mono">step &gt; stop</span> → 置 <span class="mono">out_of_steps</span>。这就是第 7 课 <span class="mono">recursion_limit</span> 的<strong>兜底实现</strong>。</li>
  </ul>
</div>
...
<div class="card detail">
  ...
  <p>第 13 课你见过 <span class="mono">messages: Annotated[list[AnyMessage], add_messages]</span>。现在能说清它的实现了：
...
<p>把上面这套套回第 13 课那张图，"Agent 循环"就清清楚楚是<strong>一串超步</strong>：</p>
<div class="vflow">
...
<div class="card detail">
  ...
  于是它们在<strong>同一个超步内并发执行</strong>、再一起结算。<span class="mono">Send</span> 的细节留到第 26 课。
</div>
  ...
  <div class="acc-body">
    <div class="qa">
      ...
      <div class="a"><span class="mono">stream_mode="updates"</span>：每个超步吐出<strong>被更新节点的输出</strong>；
        <span class="mono">"values"</span>：每步吐出<strong>整份状态</strong>；<span class="mono">"messages"</span>：逐 token。呼应第 14、19 课。</div>
    </div>
    <div class="qa">
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">你的代码</span><span class="ln">checkpointer + thread_id（承接第 13 课，这里看实现）</span></div>
...
<div class="card warn">
  ...
  原则：<strong>把副作用放在 <span class="mono">interrupt()</span> 之后</strong>，或做幂等。第 7 课的 <span class="mono">HumanInTheLoopMiddleware</span> 正是基于这套机制。
</div>

<h2>控制流原语：Send 与 Command（实现细节）</h2>
<p>第 13 课点过名，这里看它们在 <span class="mono">types.py</span> 里到底是什么、能干什么。</p>
<div class="cols">
  <div class="col">
...
<div class="card detail">
  ...
  第 21 课对照 AutoGen 时提到的"<strong>交接（handoff）</strong>"，落到 LangGraph 就是 <span class="mono">Command(goto="另一个 agent", update=...)</span>：
