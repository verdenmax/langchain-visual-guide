...
shell.code_walkthrough(
    ...
    "model_tool_loop",
    '''messages = [HumanMessage("订单 A100 现在到哪了？")]
...
<p>从学习角度看，第一课并不是要求你马上读懂所有源码，而是给后面二十八课提供坐标。遇到消息，想 BaseMessage；遇到模型，想 BaseChatModel 和 provider wrapper；遇到组合，想 Runnable；遇到循环，想 StateGraph；遇到可观测性，想 callbacks 和 config。坐标越稳，新 API 越不容易让你迷路。</p>
...
<div class="layers">
  ...
  <div class="layer l-part"><div class="lh"><span class="badge">graph</span><span class="name">langgraph</span></div><div class="ld">有状态图、节点、边、Pregel 执行、checkpoint、中断与人审。Agent 循环越来越多地落在这一层。</div></div>
...
shell.lesson_map("一次 invoke 的六个检查点", [
    ...
    ("回调", "on_chat_model_start / end / error 形成可观测生命周期", "source"),
...
shell.source_map([
    ...
    {"file": "libs/core/langchain_core/callbacks/manager.py", "symbol": "CallbackManagerForLLMRun", "role": "管理 on_llm_start/on_llm_end/on_llm_error 等生命周期事件。", "direction": "模型执行时向观察者发事件。"},
