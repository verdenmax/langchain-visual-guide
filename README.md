...
> 🌐 **在线阅读**：<https://verdenmax.github.io/langchain-visual-guide/>　·　📄 **下载 PDF（全 40 课）**：[langchain-visual-guide.pdf](https://github.com/verdenmax/langchain-visual-guide/releases/latest/download/langchain-visual-guide.pdf)
...
> 🚧 **C 级扩充中**：本教程正在从旧版 27 课扩展为约 45-55 课的中文成书级深挖版本。新版本强调更多图、worked-example trace、真实源码入口、简化伪代码和自测练习。第一到第六部分现在都是 C 级批次：全局地图、用户 API 基础、Runnable 与 LCEL、LangGraph 心智模型、LangGraph 执行引擎、Agent 内部；后续部分会按 Part 分批迁移。
...
## 📚 教程结构（C 级扩充中 · M1-M5 已完成 · M6 审阅中 · 当前 40 课）

> M6 后当前站点有 40 个 HTML 课程页面，并保留兼容旧链接的文件名，同时继续扩展到约 45-55 课。第一到第六部分都是新版 C 级批次；其余页面仍按旧版/迁移中结构保留。
...
### 旧版 / 迁移中页面
32. `11-chat-internals.html` **聊天模型内部** — `BaseChatModel` 调用链
33. `12-tool-internals.html` **工具调用内部** — 函数 → JSON Schema → `tool_calls`
34. `15-contributing.html` **读源码 / 调试 / 测试 / 贡献** — `uv` · 测试 · 调试 · 贡献规范
35. `17-rag.html` **RAG 检索增强** — `Document` → 切块 → `Embeddings` → `VectorStore` → `Retriever`
36. `20-capstone.html` **端到端实战：拼一个客服 Agent** — prompts + RAG + 工具 + 中间件
37. `21-langchain-vs-autogen.html` **LangChain vs AutoGen** — 图/管道 vs 多 Agent 对话
38. `22-ai-stack.html` **AI 全栈坐标系** — Agent 编排 5 流派 · AI 全栈 7 层
39. `23-learning-map.html` **隔壁层学习地图** — L5 推理 · L6 向量检索
40. `27-glossary.html` **术语表 · 概念索引** — 全书术语一句话查
...
├── lessons/                ← 40 课图解页面（C 级扩充中，M1-M5 已完成，M6 审阅中）
...
`print.html` 把全部 40 课合成单页文档（折叠卡片全部展开、自动分页）。用任意无头浏览器打 PDF，例如：
